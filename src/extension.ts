		...
		cp.exec(`git -C ${currentDir} diff`, (err, stdout, stderr) => {
				...
				stdout.split('\n').forEach((line) => {
						...
						if (ORIGINAL_FILE_REGEX.test(line)) {
							...
							let line_text = line.match(ADDED_LINE_REGEX)![1];
							if (line_text.toLowerCase().includes(SEARCH_WORD)) {
								curLines.push(new GiToDoLineTreeItem(curFile, line_text, curLineNumber, currentDir));
							}
							curLineNumber++;
	...
	constructor(file: string, line: string, lineNumber: number, dir: string) {
		...
		if (trimmed.length > resultLength) {
			if (index > resultLength - SEARCH_WORD.length) {
				...
				result += trimmed.substring(start_index);
			} else {
				result = trimmed;
			}
		} else {
		...
		this.label = {
			label: prefix + result,
			highlights: [[prefix.length + index, prefix.length + index + SEARCH_WORD.length]]
		};
		this.line = line;
		this.lineNumber = lineNumber;

		let open = dir + "/" + file + "#" + lineNumber;
...
class GiToDoProvider implements vscode.TreeDataProvider<GiToDoTreeItem> {
	...
	getChildren(element?: GiToDoTreeItem): vscode.ProviderResult<GiToDoTreeItem[]> {
		if (element) {
			if (element.isFile) {
	...
	private _onDidChangeTreeData: vscode.EventEmitter<GiToDoTreeItem | undefined | null | void> =
		new vscode.EventEmitter<GiToDoTreeItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<GiToDoTreeItem | undefined | null | void> =
